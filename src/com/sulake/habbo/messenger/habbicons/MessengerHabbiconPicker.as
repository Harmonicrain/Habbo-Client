package com.sulake.habbo.messenger.habbicons
{
    ...
    import com.sulake.habbo.ui.widget.chatinput.habbiconselector.HabbiconSelectorEntry;
    import com.sulake.habbo.ui.widget.chatinput.habbiconselector.HabbiconSelectorSection;
    ...
    public class MessengerHabbiconPicker
    {
        ...
        public function MessengerHabbiconPicker(window:IWindowContainer, controller:HabbiconController, selectCallback:Function)
        {
            this._window = window;
        ...
        public function toggle(anchor:IWindow):void
        {
            if (this.visible)
            {
                this.hide();
        ...
        public function show(anchor:IWindow):void
        {
        ...
        private function addOwnedSetSections():void
        {
            ...
            for each (collection in this._controller.shopCollections)
            {
                ...
                if (collection != null && collection.rewardHabbiconId > 0)
                {
                ...
                if (collection != null && entries.length > 0)
                {
        ...
        private function createTile(entry:HabbiconSelectorEntry, template:IWindowContainer):IWindowContainer
        {
            ...
            if (entry != null)
            {
                ...
                tile.name = "messenger_habbicon_item_" + entry.habbiconId;
        ...
        private function alignTo(anchor:IWindow):void
        {
            var rect:Rectangle;
            if (this._window == null || anchor == null)
            {
                return;
            }
            rect = new Rectangle();
            anchor.getGlobalRectangle(rect);
            this._window.x = rect.right - this._window.width;
            this._window.y = rect.top - this._window.height - 4;
            if (this._window.x < 0)
            {
                this._window.x = 0;
            }
        ...
        private function onTile(event:WindowEvent, window:IWindow):void
        {
            ...
            {
                this._selectCallback(entry.habbiconId, event is WindowMouseEvent && WindowMouseEvent(event).shiftKey);
            }
        }

        private function onOpenHub(event:WindowEvent, window:IWindow):void
        {
            if (event.type == WindowMouseEvent.CLICK && this._controller != null)
            {
                this._controller.resetUnseenHabbicons();
